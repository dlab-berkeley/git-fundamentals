...
read -s -p 'Make up a throwaway password: ' PASS; echo; docker rm -f rstudio 2>&1 >/dev/null; docker run -v $(pwd):/home/rstudio --name rstudio -d -p 8080:8787 -e PASSWORD="$PASS" rocker/rstudio > /dev/null
